...
def test_inventory_reads_the_registry_live_not_a_snapshot() -> None:
    """The self-healer replaces a recovered module's instance wholesale.

    The reporters hold this mapping by reference, so a replacement must be picked up
    on the next call — a snapshot would keep reporting a dead instance's tool set.
    """
    instances: dict = {"vikunja-mcp": _FakeProxy(["a", "b"])}
    health = {"vikunja-mcp": {"status": "running"}}
    assert _build_tool_inventory(instances, health)["vikunja-mcp"]["tool_count"] == 2

    instances["vikunja-mcp"] = _FakeProxy(["a", "b", "c"])
    assert _build_tool_inventory(instances, health)["vikunja-mcp"]["tool_count"] == 3
